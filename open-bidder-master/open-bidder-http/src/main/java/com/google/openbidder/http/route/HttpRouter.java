...
public class HttpRouter implements HttpReceiver {
  ...
  private static final Set<String> NO_METHODS = ImmutableSet.of();

  private final RowSortedTable<PathMatcher, String, HttpRoute> routeTable = TreeBasedTable.create();
  ...
  public HttpRouter(
    ...
    for (HttpRoute httpRoute : httpRoutes) {
      if (httpRoute.hasRequiredFeatures(enabledFeatures) && !httpRoute.getMethods().isEmpty()) {
        ...
        routeTable.put(matcher, "OPTIONS", new HttpRoute("default_options", HttpRoute.OPTIONS,
            DefaultPathMatcher.DEFAULT_PATH, optionsReceiver, ImmutableSet.<Feature>of()));

        for (String method : httpRoute.getMethods()) {
          HttpRoute oldRoute = routeTable.put(matcher, method, httpRoute);
  ...
  public final Set<String> getAllowedMethods(String requestPath) {
    for (PathMatcher pathMatcher : routeTable.rowKeySet()) {
      if (pathMatcher.apply(requestPath)) {
  ...
  protected final Set<String> getAllowedMethods(PathMatcher pathMatcher) {
