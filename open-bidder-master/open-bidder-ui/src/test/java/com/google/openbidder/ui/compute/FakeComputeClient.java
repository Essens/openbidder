...
public class FakeComputeClient extends AbstractComputeClient {
  ...
  private static final Function<AccessConfig, AccessConfig> ACCESS_CONFIG_SUMMARY =
      new Function<AccessConfig, AccessConfig>() {
        @Override
        public AccessConfig apply(AccessConfig accessConfig) {
  ...
  private Table<ComputeResourceType, String, GenericData> resources =
      HashBasedTable.create();

  // to mimic async Operations
  private Map<BigInteger, DeferredAction<? extends GenericData>> deferredActions =
      new HashMap<>();

  // resource type specific quotas
  private Map<ResourceQuotaType, Integer> quotas = new HashMap<>();
  ...
  public Operation awaitCompletion(
    ...
    Preconditions.checkNotNull(deferredAction);
  ...
  public void addAllZones(Zone... zones) {
    addAllZones(Arrays.asList(zones));
  }
  ...
  public void setQuota(ComputeResourceType resourceType, int limit) {
    Preconditions.checkNotNull(resourceType);
    ResourceQuotaType resourceQuotaType = QUOTA_TYPES.get(resourceType);
    Preconditions.checkNotNull(resourceQuotaType);
  ...
  private <T extends GenericData> Operation defer(
    ...
    if (errors.length > 0) {
      ...
      error.setErrors(Arrays.asList(errors));
