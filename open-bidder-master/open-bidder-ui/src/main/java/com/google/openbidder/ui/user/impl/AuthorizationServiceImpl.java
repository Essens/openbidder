...
public class AuthorizationServiceImpl implements AuthorizationService {
  ...
  public AuthorizationServiceImpl(
    ...
    this.userInfoUrl = Preconditions.checkNotNull(userInfoUrl);
  }
  ...
  public AuthorizationCodeFlow initializeFlow(Project project) {
    ...
    return new GoogleAuthorizationCodeFlow.Builder(
        ...
        .setCredentialStore(new UserPreferenceCredentialStore(project.getId()))
        .build();
  }
  ...
  private UserPreference revokeTokens(final ProjectUser projectUser) {
    return userPreferenceDao.updateUserPreferences(projectUser.getEmail(),
        new Transactable<UserPreference, UserPreference>() {
          @Override
          public UserPreference work(UserPreference item, Objectify ofy) {
            item.getUserRole(projectUser.getProject().getId()).deleteCredentials(null);
  ...
  private class UserPreferenceCredentialStore implements CredentialStore {

    private final long projectId;

    private UserPreferenceCredentialStore(long projectId) {
      this.projectId = projectId;
    }

    @Override
    public boolean load(String userId, Credential credential) {
      UserPreference userPreference = userPreferenceDao.getUserPreferences(userId);
      return userPreference.getUserRole(projectId).loadCredentials(credential);
    }

    @Override
    public void store(String userId, final Credential credential) {
      userPreferenceDao.updateUserPreferences(userId, new Transactable<UserPreference, Void>() {
    ...
    @Override
    public void delete(String userId, final Credential credential) {
      userPreferenceDao.updateUserPreferences(userId, new Transactable<UserPreference, Void>() {
        @Override public @Nullable Void work(UserPreference item, Objectify ofy) {
          item.getUserRole(projectId).deleteCredentials(credential);
          ofy.save().entity(item);
          return null;
        }
      });
