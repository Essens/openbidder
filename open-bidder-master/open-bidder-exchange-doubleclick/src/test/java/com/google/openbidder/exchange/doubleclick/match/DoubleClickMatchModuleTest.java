...
/**
 * Tests for {@link DoubleClickMatchModule}.
 */
public class DoubleClickMatchModuleTest {
  ...
  private void testModule(boolean enabled) {
    List<Module> modules = ImmutableList.of(
        ...
        new DoubleClickMatchModule());
    JCommander jcommander = new JCommander(modules);
    jcommander.parse(
        enabled ? "--doubleclick_match_nid=NID" : "",
        enabled ? "--doubleclick_match_url=http://match" : "",
        enabled
            ? "--doubleclick_match_interceptors=" + CountingMatchInterceptor.class.getName()
            : "");
