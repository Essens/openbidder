...
public class RemarketingBidInterceptor implements BidInterceptor {
  ...
  private static final List<ObExt.Bid.UrlParameter> IMPRESSION_PARAMS =
      ImmutableList.of(ObExt.Bid.UrlParameter.newBuilder()
  ...
  public void execute(InterceptorChain<BidRequest, BidResponse> chain) {
    ...
    if (Iterables.size(actions) > 0) {
      ...
      Impression imp = chain.request().openRtb().getImp(0); //TODO(opinali): why only the first?
      ...
      if (winningAction != null) {
        Bid.Builder newBid = Bid.newBuilder()
            ...
            .setExtension(ObExt.bid, ObExt.Bid.newBuilder()
                .addClickThroughUrl(winningAction.getClickThroughUrl())
                .addAllImpressionParameter(IMPRESSION_PARAMS).build());
