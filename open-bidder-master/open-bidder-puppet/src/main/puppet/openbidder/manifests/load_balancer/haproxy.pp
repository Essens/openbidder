...
class openbidder::load_balancer::haproxy (
  ...
  file { 'rsyslog_haproxy':
    content => template('openbidder/load_balancer/rsyslog_haproxy.conf.erb'),
    ensure => file,
  ...
  file { 'rsyslog_default_config':
    ensure => file,
    path => '/etc/rsyslog.d/50-default.conf',
    source => 'puppet:///modules/openbidder/load_balancer/rsyslog_50-default.conf',
    mode => 0644,
    owner => 'root',
    group => 'root',
  ...
  service { 'rsyslog':
    ...
    subscribe => [File['rsyslog_haproxy'], File['rsyslog_default_config']],
  }
  ...
  service { 'haproxy':
    ...
    require => [File['haproxy'], File[$chroot_dir]],
  }
}
