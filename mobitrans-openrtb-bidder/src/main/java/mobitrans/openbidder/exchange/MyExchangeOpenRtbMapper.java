  ...
  @Override public OpenRtb.BidRequest.Builder toOpenRtbBidRequest(String nativeRequest) {
    // Request format: "id;width;height" -- supports a single impression
           ...
           OpenRtbJsonFactory openrtbJsonFactory = OpenRtbJsonFactory.create();
            ...
            OpenRtbJsonReader jsonReader =  openrtbJsonFactory.newReader();
            
           BidRequest requestObj = null;
      try {
          requestObj = jsonReader.readBidRequest(nativeRequest);
      } catch (IOException ex) {
          Logger.getLogger(MyExchangeOpenRtbMapper.class.getName()).log(Level.SEVERE, null, ex);
      }
           
           return requestObj.toBuilder();
//                Gson gson = new Gson();
//                //fromJson
