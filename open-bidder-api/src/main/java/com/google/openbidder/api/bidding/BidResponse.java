...
public class BidResponse extends UserResponse<BidResponse> {
   ...
   * @throws IllegalStateException if the response builder was already set to OpenRTB mode.
   */
  ...
  /**
   * Returns the {@link ResponseMode}, which tells you which of {@link #openRtb()} or
   * {@link #nativeResponse()} can be invoked safely.
   */
  public final ResponseMode getResponseMode() {
    return response != null
        ? ResponseMode.OPENRTB
        : nativeResponse != null
            ? ResponseMode.NATIVE
            : ResponseMode.NONE;
  }
  ...
  public static class Builder extends UserResponse.Builder<Builder> {
    ...
    @Override public BidResponse build() {
  ...
  /**
   * The bid response builder will be in either "native mode" or "openrtb mode", mutually exclusive.
   * The first time you use any of the response builder accessors, {@link BidResponse#openRtb()}
   * or {@link BidResponse#nativeResponse()}, the response mode will be set accordingly.
   * Mixed usage is illegal, so if you e.g. set the response to native mode and then try to
   * obtain the OpenRTB-format response (directly, or indirectly via methods that depend on that),
   * an {@link IllegalStateException} will be thrown.
   */
  public static enum ResponseMode {
    /** No builder was set yet; any response builder accessor can be used. */
    NONE,
    /** OpenRTB builder is already set; only {@link BidResponse#openRtb()} can be used. */
    OPENRTB,
    /** Native builder is set; only {@link BidResponse#nativeResponse()} can be used. */
    NATIVE,
  }
}
