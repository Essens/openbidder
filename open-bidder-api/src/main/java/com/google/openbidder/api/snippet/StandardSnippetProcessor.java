...
public class StandardSnippetProcessor extends OpenRtbSnippetProcessor {
      ...
      switch ((SnippetMacros) macroDef) {
        ...
        case OB_IMPRESSION_URL:
          if (Strings.isNullOrEmpty(impressionUrl)) {
            ...
            generateUrl(
                impressionUrl, ctx.bid().getExtension(ObExt.bidImpressionParameter), sb);
          }
          break;
        ...
        case OB_CLICK_URL:
          if (Strings.isNullOrEmpty(clickUrl)) {
            ...
            generateUrl(
                clickUrl, ctx.bid().getExtension(ObExt.bidClickParameter), sb);
          }
          break;

        case OB_AD_CLICKTHROUGH_URL:
          if (ctx.bid().getExtensionCount(ObExt.bidClickThroughUrl) == 0) {
            throw new UndefinedMacroException(SnippetMacros.OB_AD_CLICKTHROUGH_URL,
                "Bid's ad clickThroughUrl is not set");
          } else {
            sb.append(ctx.bid().getExtension(ObExt.bidClickThroughUrl).get(0));
          }
          break;
        ...
        case OB_AD_WIDTH: {
          if (ctx.bid().hasW()) {
            ...
            ImpOrBuilder imp = findImp(ctx, SnippetMacros.OB_AD_WIDTH);
        ...
        case OB_AD_HEIGHT: {
          if (ctx.bid().hasH()) {
            ...
            ImpOrBuilder imp = findImp(ctx, SnippetMacros.OB_AD_HEIGHT);
            sb.append(imp.hasBanner()
  ...
  protected void generateUrl(
      String url, List<? extends ObExt.UrlParameter> parameters, StringBuilder sb) {
    sb.append(url);

    if (!parameters.isEmpty()) {
      char sep = url.lastIndexOf('?') == -1 ? '?' : '&';

      for (ObExt.UrlParameter entry : parameters) {
