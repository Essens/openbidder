...
public class NettyClientFactory<M> extends AbstractIdleService {
  ...
  public NettyClientFactory(
    ...
    this.clientGroup = checkNotNull(clientGroup);
    this.clientChannel = checkNotNull(clientChannel);
  ...
  public NettyClient<M> connect() throws Exception {
    checkNotNull(defaultUri, "Default URL must be set for calling connect()");
