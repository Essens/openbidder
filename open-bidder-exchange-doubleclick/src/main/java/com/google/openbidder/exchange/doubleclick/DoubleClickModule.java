...
public class DoubleClickModule extends AbstractModule {
  ...
  private final String path = DoubleClickBidRequestPath.DEFAULT;
  ...
  @Parameter(names = "--doubleclick_local_resources", arity = 1,
      description = "Load DoubleClick dictionaries from local resources (test only)")
  private final boolean localResources = false;

  @Parameter(names = "--doubleclick_openrtb", arity = 1,
      description = "Enable OpenRTB mapping")
  private final boolean openRtb = true;

  @Parameter(names = "--doubleclick_validate", arity = 1,
      description = "Validate response messages")
  private final boolean validate = false;

  @Parameter(names = "--doubleclick_extmappers",
      description = "ExtMappers to be used with the DoubleClick/OpenRTB mapper")
  private String extMapperNames;
  ...
  protected void configure() {
    ...
    if (doubleClickBiddingEnabled) {
      ...
      TypeLiteral<OpenRtbMapper<BidRequest, BidResponse, BidRequest.Builder, BidResponse.Builder>>
          mapperTL = new TypeLiteral<OpenRtbMapper<
              BidRequest, BidResponse, BidRequest.Builder, BidResponse.Builder>>() {};
      if (openRtb) {
        bind(mapperTL).to(DoubleClickOpenRtbMapper.class);
      } else {
        bind(mapperTL).toInstance(NullDoubleClickOpenRtbMapper.INSTANCE);
      }
      ...
      ImmutableList.Builder<ExtMapper> extMappers = ImmutableList.<ExtMapper>builder()
          .add(DoubleClickLinkMapper.INSTANCE)
          .add(OpenBidderExtMapper.INSTANCE);
      if (extMapperNames != null) {
        for (String extMapperName : extMapperNames.split(",")) {
          try {
            extMappers.add((ExtMapper) Class.forName(extMapperName).newInstance());
          } catch (ReflectiveOperationException e) {
            logger.info("Cannot create ExtMapper: {}", e.toString());
          }
        }
      }
      bind(new TypeLiteral<List<ExtMapper>>() {}).toInstance(extMappers.build());
    } else {
  ...
  protected static class HttpRouteProvider extends AbstractHttpRouteProvider {
    @Inject
  ...
  public OpenRtbJsonFactory provideOpenRtbJsonFactory(JsonFactory jsonFactory) {
    return BidModule.registerObExt(OpenRtbJsonFactory.create().setJsonFactory(jsonFactory));
  }
}
