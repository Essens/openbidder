  ...
  public void receive(HttpReceiverContext ctx) {
    ...
    try {
      ...
      if (logger.isDebugEnabled()) {
        ...
        if (dcRequest.hasId()) {
          setRequestId(BaseEncoding.base64Url().omitPadding().encode(
              dcRequest.getId().toByteArray()));
        }
        logger.debug("DoubleClick request: {}", TextFormat.shortDebugString(dcRequest));
        logger.debug("Open Bidder Request: {}", request);
      }
      ...
      if (logger.isDebugEnabled()) {
        logger.debug("Open Bidder response: {}", response);
        logger.debug("DoubleClick response: {}", TextFormat.shortDebugString(dcResponse));
      }
      ...
      logger.error("Bad DoubleClick request: {}", e.toString());
  ...
  NetworkBid.BidResponse.Builder handleBidRequest(BidRequest request, BidResponse response) {
    ...
    try {
      ...
      if (response.getResponseMode() == BidResponse.ResponseMode.OPENRTB) {
        ...
        snippetProcessor.process(openrtbRequest, openrtbResponse);
        dcResponse = mapper.toExchangeBidResponse(openrtbRequest, openrtbResponse.build());
      } else { // NATIVE or NONE
