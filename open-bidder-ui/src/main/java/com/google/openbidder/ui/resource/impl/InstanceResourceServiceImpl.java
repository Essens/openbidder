...
public class InstanceResourceServiceImpl
  ...
  private static final int MAX_INSTANCE_WAIT_SEC = 120;
  ...
  public InstanceResourceServiceImpl(
    ...
    this.bidderInstanceBuilder = checkNotNull(bidderInstanceBuilder);
    this.loadBalancerInstanceBuilder = checkNotNull(loadBalancerInstanceBuilder);
  ...
  private Instance buildBidder(
    ...
    if (project.getBidderMachineType(zoneName) == null
      ...
      project = getProjectService().updateProject(projectId, new Function<Project, Project>() {
  ...
  private InstanceResource startInstance(
    ...
    List<AttachedDisk> disks = ImmutableList.of(
        new AttachedDisk()
            ...
            .setType(DISK_TYPE)
            .setSource(checkNotNull(disk.getSelfLink()))
            .setKind(DISK_KIND)
            .setMode(DISK_MODE)
    ...
    for (int secondsToWait = 0; secondsToWait <= MAX_INSTANCE_WAIT_SEC
    ...
    if (finalInstance == null) {
      throw new ComputeResourceNotFoundException(ResourceName.buildName(instance));
    }
