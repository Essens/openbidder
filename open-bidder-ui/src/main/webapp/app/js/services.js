 ...
 * </ul>
 */
openBidderServices.factory('Project', [
    '$http', '$q', '$location', '$rootScope', 'User', 'Quota', 'Region', 'Network', 'Firewall',
    'Zone', 'Instance', 'MachineType', 'Image', 'Report', 'Account', 'Remarketing',
    function($http, $q, $location, $rootScope, User, Quota, Region, Network, Firewall,
        Zone, Instance, MachineType, Image, Report, Account, Remarketing) {
...
openBidderServices.factory('Instance', ['$http', function($http) {
  ...
  Instance.list = function(projectId, zoneId, opt_params) {
    return $http.get('/projects/' + encodeUriSegment(projectId) + '/zones/' +
        encodeUriSegment(zoneId) + '/instances', {
      params: opt_params
    }).then(function(response) {
  ...
  Instance.create = function(projectId, zoneId, opt_params) {
    ...
    return $http.post('/projects/' + encodeUriSegment(projectId) + '/zones/' +
        encodeUriSegment(zoneId) + '/instances', {
...
openBidderServices.factory('MachineType', ['$http', function($http) {
  ...
  MachineType.list = function(projectId, zoneId, opt_params) {
    var uri = '/projects/' + encodeUriSegment(projectId) +
      '/zones/' + encodeUriSegment(zoneId) + '/machinetypes';
    return $http.get(uri, {
      params: opt_params || {}
    }).then(function(response) {
     ...
     handle: function(data) {
       ...
       switch (data.status) {
         ...
         case HttpStatus.PRECONDITION_FAILED:
           NoticeQueue.err('Your Google API Project ID is incorrect' +
 ...
 */
openBidderServices.factory(
    'Channel', ['$rootScope', '$http', '$routeParams', '$q', '$timeout', 'NoticeQueue',
    function($rootScope, $http, $routeParams, $q, $timeout, NoticeQueue) {
  var socket;
 ...
 */
openBidderServices.factory('errorHandlingInterceptor', [
    '$q', 'NoticeQueue', function($q, NoticeQueue) {
  return {
