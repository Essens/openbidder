...
NewProjectController.$inject = [
  '$scope', '$location', 'Project', 'HttpErrorHandler'
];
...
ProjectDeleteController.$inject = [
...
NetworkController.$inject = [
  '$scope', '$routeParams', '$http', 'Project', 'Network', 'Firewall',
  'HttpErrorHandler', 'NoticeQueue'
];
 ...
 */
function BidderProfileController(
    $scope, $routeParams, Project, MachineType, Image, NoticeQueue, HttpErrorHandler) {
  ...
  $scope.createCustomBidder = function() {
    if ($scope.machineTypeModel && $scope.imageModel) {
      Project.get($routeParams.projectId).then(function(project) {
        ...
        if (error.status == HttpStatus.NOT_FOUND) {
          NoticeQueue.err('rror creating instances.');
        } else {
          HttpErrorHandler.handle(error);
        }
      });
...
BidderProfileController.$inject = [
  '$scope', '$routeParams', 'Project', 'MachineType', 'Image', 'NoticeQueue', 'HttpErrorHandler'
];
...
InstanceDetailsController.$inject = [
  '$scope', '$routeParams', 'Project', 'HttpErrorHandler'
];
...
UserController.$inject = [
  '$scope', '$routeParams', 'HttpErrorHandler', 'Project', 'NoticeQueue', 'UserEmail'
];
...
PreferredDealsController.$inject = [
  '$scope', '$routeParams', 'Project', 'HttpErrorHandler', 'NoticeQueue'
];
 ...
 */
function RemarketingController(
    $scope, $routeParams, Project, Remarketing, HttpErrorHandler, NoticeQueue) {
  $scope.routeParams = $routeParams;
  Project.get($routeParams.projectId).then(function(project) {
    $scope.project = project;
